        ...
        "inputs": [
            {
                "components": [
                    ...
                    {
                        "internalType": "uint256[]",
                        "name": "_instructionParentIds",
                        "type": "uint256[]"
                    },
                    {
                        "components": [
                            {
                                "internalType": "enum UniversalInterpreter.CallType",
                                "name": "callType",
                                "type": "uint8"
                            },
                            {
                        ...
                        "internalType": "struct UniversalInterpreter.CallInstruction[]",
                        "name": "_allInstructions",
                        "type": "tuple[]"
                    }
                ],
                "internalType": "struct UniversalInterpreter.AttackParameters",
                "name": "params",
                "type": "tuple"
            }
        ],
...
func main() {
	...
	jsonData, err := ioutil.ReadFile("go-executor/attack.json")
	if err != nil {
	...
	privateKeyHex := "YOUR_PRIVATE_KEY" // e.g. "ac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80"
	rpcEndpoint := "YOUR_RPC_ENDPOINT" // e.g. "http://127.0.0.1:8545"
	...
	// Create transaction
	tx, err := auth.Signer(auth.From, &bind.TransactOpts{
		From:     auth.From,
		Nonce:    auth.Nonce,
		Signer:   auth.Signer,
		Value:    auth.Value,
	...
	}, &contractAddress, packedBytes)


	if err != nil {
		log.Fatalf("Failed to create raw transaction: %v", err)
	}
