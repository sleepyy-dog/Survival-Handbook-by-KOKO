...
$$
该统计量服从自由度为 $n-1$ 的 $t$ 分布，即 $T \sim t(n-1)$（虽然我们此时变化的是$\bar{X}$，但是我们最后将$\bar{X}$确定，来求$u$的区间）（$T$分布的成立前提是我们的$X$服从正态分布）(这里虽然是从n出发，但是从我们以下的推导可以看出自由度是$n-1$)。
...
> [!NOTE]
>
> 这里的样本方差直接换成总体方差也是可以的，因为本来就是近似，后面我们的一些计算事实上就是直接用的总体方差。
