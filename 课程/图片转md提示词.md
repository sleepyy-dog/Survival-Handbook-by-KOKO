...
* **行间公式**: 独立成行的数学公式，必须使用**双美元符号**包裹，格式为 `$$...$$`（注意独立成行的行内公式默认为行间公式）（对于出现标号的行间公式，删掉标号，如果此标号在后续文章中有引用，给对应公式打上label，使用\ref引用）。
...
2. **逐段精细化处理**: 按从上到下的顺序，逐段处理识别出的内容：
   ...
   d. **翻译并格式化公式**: 对识别出的数学公式，先将其视觉结构翻译成LaTeX代码，然后根据其是行内还是行间，分别使用 `$...$` 或 `$$...$$` 进行包裹。（对于出现标号的行间公式，删掉标号，如果此标号在后续文章中有引用，给对应公式打上label，使用\ref引用）

   e.**对于题目或者命题使用相应的模板**，模板如下：

     ```markdown
     > #### ** (题目来源（P84 第九题）或内容归类（命题名字）):**
     >
     > 具体内容...
...
**在图片只包含单独题目或者命题时只需使用上述模板中的引用部分的内容，若包含（题目和解答）或者（命题和证明）则使用以上的完整模板**
...
* **Example 1**
  **用户输入**: 一张图片，图片上手写内容如下：
  ...
  selector = Keccak("transfer(address,uint256)")[0:4]    （1）

  1+1=2

  从以上（1）可以看出这个selector是唯一的。
  ...
  **你的输出**:
  ...
  ```markdown
  在以太坊中，$ABI$编码定义了如何与合约交互。函数选择器$selector$是函数签名的$Keccak$哈希的前4个字节。
  ...
  从以上$\ref{selector}$可以看出这个$selector$是唯一的。
  ...
  *(示例注释：此示例完整体现了[Workflows]的全过程。AI正确识别并应用了所有规则：1. 将中文句子中的英文术语`ABI`、`selector`、`Keccak`以及函数签名`transfer(address, uint256)`作为行内元素，用`$...$`包裹。2. 将独立成行的公式作为行间公式，用`$$...$$`包裹，并调整引用索引。3. 将文本中的句号和逗号转换为中文全角符号`。`和`，`。4. 对于题目使用模板5.按照原始顺序整合所有元素，输出纯净的Markdown。)*
